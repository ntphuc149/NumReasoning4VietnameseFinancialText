...
#   * gpt-oss-120b / gpt-oss-20b: probed directly against this endpoint
#     (2026-08-18). gpt-oss-120b at max_tokens=64 and 128 returned
#     finish_reason="length" with content="" and a populated hidden trace;
#     at 768 it still came back finish_reason="length" on a moderate
#     financial-reasoning prompt. Its trace is NOT under `reasoning_content`
#     -- see HIDDEN_REASONING_FIELDS below.
...
# Which message field holds the hidden chain-of-thought. There is no single
# convention: DeepSeek-V4-Flash and GLM-5.2 use `reasoning_content` (verified
# in this repo's rate-check notebook), while gpt-oss-120b on this endpoint
# uses plain `reasoning` -- its `reasoning_content` is always None, measured
# directly. Checking only `reasoning_content` silently disables the
# starvation escalation below for the gpt-oss family: content is empty, the
# detector sees None, and every retry re-runs at the identical budget until
# max_retries is exhausted and LLMError is raised.
HIDDEN_REASONING_FIELDS = ("reasoning_content", "reasoning")


def hidden_reasoning(message) -> str:
    """The model's hidden trace, whichever field this provider puts it in."""
    for field in HIDDEN_REASONING_FIELDS:
...
class LLMClient:
    ...
    def _content(choice) -> str:
        ...
        A reasoning model keeps its chain-of-thought in a separate field
        (`reasoning_content` for GLM-5.2 and DeepSeek-V4-Flash, `reasoning`
        for gpt-oss -- see HIDDEN_REASONING_FIELDS), which the API already
        keeps out of `message.content`. So the final program/answer is
        `content`, with no stripping needed, on every provider seen so far.
        """
        ...
        for attempt in range(1, cfg.max_retries + 1):
            ...
            for choice in response.choices:
                ...
                if not content:
                    ...
                    # non-empty hidden trace behind, the API's own signal
                    # that generation was still inside that trace when
                    # max_tokens ran out. The field name differs per provider
                    # -- see HIDDEN_REASONING_FIELDS.
                    message = getattr(choice, "message", None)
                    if hidden_reasoning(message):
