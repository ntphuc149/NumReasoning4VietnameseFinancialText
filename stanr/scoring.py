...
Vendored verbatim from `notebooks/evaluate/scorer.py` (the corrected,
validated implementation this repo's paper results are computed with -- see
that file's docstring for the five corrections applied over the original
FinQA script). Kept as a copy rather than an import because notebooks in this
repo already inline this same file for the same reason (Kaggle/Modal sessions
can't import from the repo), so `stanr` follows the established convention
instead of introducing a new cross-directory import path.

`ReasoningDistiller` uses `equal_program`/`program_tokenization` to decide
which distilled traces to keep; `STaNRTrainer` uses `evaluate_dataframe`/
`score_one` for both the self-teaching filter and PA/EA reporting.
"""
...
def program_tokenization(original_program: str) -> List[str]:
    ...
    Raises ValueError on trailing unparsed content (e.g. a truncated
    generation missing its closing paren) rather than silently returning a
    partial parse.
    """
...
def equal_program(program1: List[str], program2: List[str]) -> bool:
    ...
    Literals become symbols, table steps become opaque variables, and the two
    expressions are compared after `simplify`, so a differently-arranged but
    algebraically identical program still counts. A prediction may only use
    symbols that appear in gold, so it cannot introduce a constant of its own
    (the `100` of a percentage rescaling, say).
    """
    try:
...
def score_one(generated_program: str, gold_program: str, gold_answer,
    ...
    A generated_program that fails to tokenize scores (0.0, 0.0) rather than
    raising -- expected input from a real model, not a bug to surface.
    """
